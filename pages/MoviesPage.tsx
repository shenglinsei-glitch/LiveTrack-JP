...
const MOVIE_STATUSES: MovieStatus[] = ['未上映', '上映中', '鑑賞済み', '見送り', '上映終了'];
...
export const MoviesPage: React.FC<MoviesPageProps> = ({ movies, onOpenDetail, onExport, onImport, isMenuOpenExternally, onMenuClose, hideHeader = false }) => {
  ...
  const displayMovies = useMemo(() => {
    ...
    const getDate = (movie: Movie) => dayjs(movie.watchDate || movie.releaseDate || '2999-12-31').valueOf();
                  ...
                  >
                    <div style={{ position: 'relative', paddingTop: '140%', background: '#F3F4F6' }}>
                      ...
                      >
                        ...
                        <div style={{ marginTop: 4, fontSize: 11, fontWeight: 700, opacity: 0.82 }}>{movie.watchDate ? `鑑賞日：${fmtDate(movie.watchDate)}` : `公開日：${fmtDate(movie.releaseDate)}`}</div>
                      </div>
                    </div>
                  </GlassCard>
                </div>
