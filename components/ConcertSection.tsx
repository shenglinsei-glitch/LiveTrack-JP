                ...
                <div key={perf.id} className="bg-white/40 rounded-xl border border-white/50 overflow-hidden shadow-sm">
                  <div className="flex items-center justify-between p-2.5">
                    <div className="flex flex-col gap-0.5">
                       ...
                       {perf.status === ConcertStatus.PENDING && (perf.lotteryResultDate || (perf as any).lotteryResultName) && (
                        <div className="flex flex-col gap-0.5 mt-1">
                          <div className="flex items-center gap-2">
                            <svg className="w-3 h-3 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            {perf.lotteryResultDate ? (
                              <span className="text-[9px] md:text-[10px] font-black text-amber-600 tracking-tight">
                                抽選発表: {new Date(perf.lotteryResultDate).toLocaleString('ja-JP', { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                              </span>
                            ) : (
                              <span className="text-[9px] md:text-[10px] font-black text-amber-600 tracking-tight">
                                抽選発表: -
                          ...
                          {(perf as any).lotteryResultName ? (
                            <div className="pl-5 text-[9px] md:text-[10px] font-black text-amber-700 tracking-tight">
                              名称: {(perf as any).lotteryResultName}
                            </div>
                          ) : null}
                        </div>
                      )}</div>
