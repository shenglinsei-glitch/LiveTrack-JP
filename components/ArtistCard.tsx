          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            {relevantPerformances.map((perf) => {
              ...
              return (
                <div 
                  key={perf.id} 
                  onClick={() => isJoined && onConcertClick?.(perf.concertId)}
                  className={`flex items-center justify-between p-3 bg-gray-50/50 rounded-2xl border border-gray-100 ${isJoined ? 'cursor-pointer hover:bg-gray-100/50 transition-colors' : ''}`}
                >
